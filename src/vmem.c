...
init_kern_translation_table(void){
	...
	// TODO Remplir la second level table!
	uint8_t* first_level_table = kAlloc_aligned(FIRST_LVL_TT_SIZE, 14);
	for (int i = 0; i< __kernel_heap_end__; i++){
		uint8_t* second_level_table = kAlloc_aligned(SECON_LVL_TT_SIZE, 10);
		first_level_table = (uint32_t*)second_level_table;
		first_level_table ++;
	}

	// TODO boucler sur l'espace des IO devices (0x20x0 -> 0x20FxF)
	
	if(adresse logique){
		// Adresse phy = adresse logique
...
void vmem_init(){
uint32_t device_flags = 010000110111;
}
